...
export class ConfigurationComponent implements OnInit {
  ...
  addPersonGroup(){
    this.inputBox.show('Thêm Lớp ', 'Tên lớp:').then(result => { 
  ...
  addPerson() {
    this.inputBox.show('Thêm sinh viên', 'Tên sinh viên:').then(result => {
  ...
  addPersonFace() {
    this.inputBox.show('Thêm ảnh', 'URL:').then(result => {
  ...
  trainPersonGroup() {
    ...
    this.faceApi.trainPersonGroup(this.selectedGroupId).subscribe(() => {
      this.toastr.pop('info', 'Training', 'Training...');
      this.loading = false;
    });
  }
  ...
  getGroupTrainingStatus() {
    ...
    this.faceApi.getPersonGroupTrainingStatus(this.selectedGroupId).subscribe(result => {
      switch (result.status) {
        case 'succeeded':
          this.toastr.pop('Thành Công', 'Training Thành Công');
          break;
        case 'running':
          this.toastr.pop('info', 'Đang Trainning...', 'Thử lại sau');
          break;
        case 'failed':
          this.toastr.pop('error', 'Không thể Training', result.message);
          break;
        default:
          break;
      }
